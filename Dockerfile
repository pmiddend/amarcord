...
RUN pip install  --only-binary ':all:' --no-binary 'randomname' --no-binary 'termcolor' --no-binary 'fire' --no-binary 'blinker' --no-binary 'karabo_bridge' --no-binary 'typed-argument-parser' -r requirements.txt
