...
findVariablesInMustache nodes =
    let
        findVariable : Mustache.Tag -> List String
        findVariable x =
            case x of
                Mustache.Variable _ ->
                    [ String.join "." (Mustache.name x) ]

                Mustache.Section sectionData ->
                    List.concatMap findVariable (Mustache.tags (Mustache.sectionContents sectionData))

                Mustache.InvertedSection sectionData ->
                    List.concatMap findVariable (Mustache.tags (Mustache.invertedSectionContents sectionData))

                _ ->
                    []
    in
    List.concatMap findVariable (Mustache.tags nodes)
