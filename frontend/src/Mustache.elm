...
addNode node state =
    { state | current = node :: state.current }
...
leaveSection : String -> ParserState -> ParserState
leaveSection postWhitespace state =
    case state.stack of
        ( _, ast, sectionType ) :: xs ->
            { stack = xs
    ...
    loop []
        (\lines ->
            chompUntilOneOf
                ...
                , map (always (\s -> Loop (String.slice 0 -2 s :: lines))) (token "\u{000D}\n")
...
lookup context variableName =
    case ( context, variableName ) of
        ( [], _ ) ->
            Nothing
...
interpolate m =
    D.oneOf
        ...
        , D.map (\xs -> List.map (\v -> interpolate (Just v)) xs |> String.concat) (D.list D.value)
        , D.map (\_ -> "") (D.keyValuePairs D.value)
        ]
        |> (\d ->
                D.decodeValue d (Maybe.withDefault E.null m)
...
section context path inner =
    case lookup context path of
        Nothing ->
            ""
...
invertedSection context path inner =
    case lookup context path of
        Nothing ->
            inner context
