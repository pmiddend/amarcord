...
def test_add_comments(db: DB) -> None:
    with db.connect() as conn:
        ...
        assert len(db.retrieve_run(conn, run_id).comments) == 1

        db.add_comment(conn, run_id, "author2", "text2")
        assert len(db.retrieve_run(conn, run_id).comments) == 2
...
def test_delete_comment(db: DB) -> None:
    with db.connect() as conn:
        ...
        assert not db.retrieve_run(conn, run_id).comments
...
def test_modify_comment(db: DB) -> None:
    with db.connect() as conn:
        ...
        db.add_comment(conn, run_id, "author", "text")
        c = replace(db.retrieve_run(conn, run_id).comments[0], text="modified")
        db.change_comment(conn, c)
        assert db.retrieve_run(conn, run_id).comments[0].text == "modified"
